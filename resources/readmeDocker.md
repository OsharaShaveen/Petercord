...
- Setup configs
    - Download the sample config file <br>
        - ```mkdir petercord && cd petercord```
        - ```wget https://raw.githubusercontent.com/IlhamMansiez/petercord/petercord/config.env.sample -O config.env```
        - ```vim config.env```
    - Download the yml file for docker-compose
        - ```wget https://raw.githubusercontent.com/IlhamMansiez/petercord/petercord/resources/docker-compose.yml```
