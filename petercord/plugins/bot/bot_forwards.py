...
if petercord.has_bot:
    ...
    async def ban_from_bot_pm(ban_user, reason: str, log: str = False) -> None:
        ...
        await asyncio.gather(
            ...
            petercord.bot.send_message(user_.id, banned_msg),
        )
        info = (
        ...
        if log:
            await petercord.getCLogger(log).log(info)
        return info
    ...
    async def uinfo_(_, message: Message):
        ...
        if uid_from_db := BOT_MSGS.search(reply.message_id):
            try:
                user_ = await petercord.bot.get_user_dict(uid_from_db, attr_dict=True)
            except Exception:
                pass
        elif user_from_fwd := reply.forward_from:
            user_ = await petercord.bot.get_user_dict(user_from_fwd, attr_dict=True)
