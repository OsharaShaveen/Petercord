...
@petercord.on_cmd("gogo", about={"header": "Gogo Plugin Help"})
async def gogo_h(message: Message):
    if not petercord.has_bot:
        await message.err("You Need to create a bot via Bot Father", del_in=5)
        return
    link_ = "https://t.me/{}?start=inline".format((await petercord.bot.get_me()).username)
...
if petercord.has_bot:
    ...
    @petercord.bot.on_callback_query(filters.regex(pattern="get_eps(.*)"))
    @check_owner
    ...
    @petercord.bot.on_callback_query(filters.regex(pattern=r"get_currentpg(.*)"))
    @check_owner
