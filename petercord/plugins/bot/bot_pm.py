...
if petercord.has_bot:
    ...
    async def get_bot_pm_media() -> None:
        ...
        if not Config.BOT_MEDIA:
            _BOT_PM_MEDIA = get_file_id(await petercord.bot.get_messages("useless_x", 2))
            return
            ...
            if match:
                ...
                try:
                    bot_m_fid = get_file_id(
                        await petercord.bot.get_messages(from_chat, msg_id)
                    )
    ...
    async def get_bot_info():
        ...
        if not (
            ...
            try:
                owner_info = await petercord.bot.get_user_dict(
                ...
                _CACHED_INFO["bot"] = await petercord.bot.get_user_dict(
                    "me", attr_dict=True
                )
    ...
    async def check_new_bot_user(user: Union[int, str, User]) -> bool:
        user_ = await petercord.bot.get_user_dict(user, attr_dict=True)
    ...
    def default_owner_start(from_user):
        start_msg = f"Hello PETERCORD **{from_user.flname}** !\n"
        btns = [
            [InlineKeyboardButton("➕  ADD TO GROUP", callback_data="add_to_grp")],
        ]
        return start_msg, btns

    @petercord.bot.on_message(start_filter())
    async def start_bot(_, message: Message):
        c_info = await get_bot_info()
        bot_ = c_info.get("bot")
        owner_ = c_info.get("owner")
        from_user = await petercord.bot.get_user_dict(message.from_user, attr_dict=True)
...
Nice To Meet You !, I'm <b>{bot_.fname}</b> A Bot.
        ...
        <b><i>Powered by</i> [OWNERS](https://t.me/diemmmmmmmmmm)
...
"""
    ...
    @petercord.bot.on_callback_query(filters.regex(pattern=r"^add_to_grp$"))
    @check_owner
    ...
    @petercord.bot.on_callback_query(filters.regex(pattern=r"^back_bot_pm$"))
    @check_owner
    async def back_bot_pm_(c_q: CallbackQuery):
        await c_q.answer()
        start_msg, btns = default_owner_start(
            await petercord.bot.get_user_dict(c_q.from_user, attr_dict=True)
        )
    ...
    async def send_flood_alert(user_id: Union[int, User]) -> None:
        user_ = await petercord.bot.get_user_dict(user_id, attr_dict=True)
        buttons = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
        ...
        if found:
            ...
            if not fa_id:
                return
            try:
                msg_ = await petercord.bot.get_messages(Config.LOG_CHANNEL_ID, fa_id)
            ...
            fa_msg = await petercord.bot.send_message(
    ...
    @petercord.bot.on_callback_query(filters.regex(pattern=r"^bot_pm_ban_([0-9]+)"))
    @check_owner
    ...
    @petercord.bot.on_callback_query(~FloodConfig.OWNER & BotAntiFloodFilter, group=-100)
    ...
    @petercord.bot.on_callback_query(filters.regex(pattern=r"^toggle_bot-antiflood_off$"))
    @check_owner
