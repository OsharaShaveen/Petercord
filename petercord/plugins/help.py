...
REPO_X = InlineQueryResultArticle(
    title="Repo",
    input_message_content=InputTextMessageContent("**Here's how to setup Petercord** "),
    ...
    reply_markup=InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "PETERCORD REPO", url="https://github.com/IlhamMansiez/Petercord"
                ),
                InlineKeyboardButton(
                    "DEPLOY PETERCORD",
                    url=(
                        "https://heroku.com/deploy?template="
                        "https://github.com/ilham77mansiz/deploy"
                    ),
                ),
            ]
        ]
    ...
    def check_owner(func):
        async def wrapper(_, c_q: CallbackQuery):
            if c_q.from_user and (
                ...
                await c_q.answer(
                    f"Only {user_dict['flname']} Can Access this...! Build Your petercord-X",
                    show_alert=True,
                )
