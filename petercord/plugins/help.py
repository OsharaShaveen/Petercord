...
if petercord.has_bot:
    def check_owner(func):
        async def wrapper(_, c_q: CallbackQuery):
            if c_q.from_user and c_q.from_user.id in Config.OWNER_ID:
                ...
                await c_q.answer(
                    f"Only {user_dict['flname']} Can Access this...! Build Your Own @TEAMSquadUserbotSupport 🎖",
                    show_alert=True)
        return wrapper
    ...
    def default_buttons(cur_pos: str):
        ...
        if cur_pos != "mm":
            ...
            cur_clnt = "✅ Pengguna" if Config.USE_USER_FOR_CLIENT_CHECKS else "🤖 BOT"
            tmp_btns.append(InlineKeyboardButton(
...
async def helpme(message: Message) -> None:  # pylint: disable=missing-function-docstring
    if petercord:
        ...
        try:
            nice = await message.get_inline_bot_results(bot=bot_username, query="helpme")
            await message.send_inline_bot_result(
