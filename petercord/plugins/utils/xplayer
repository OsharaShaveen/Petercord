...
class XPlayer(GroupCall):
    def __init__(self, chat_id: int):
        ...
        super().__init__(
            client=petercord, play_on_repeat=self.replay_songs, path_to_log_file=""
        )
...
async def get_groupcall(chat_id: int) -> XPlayer:
    if not vc_chats.get(chat_id):
        ...
        if petercord.has_bot:
            try:
                await petercord.get_chat_member(
...
async def play_now(gc: XPlayer) -> None:
    r = gc.playlist[0]
    key = r["id"]
    client = petercord.bot if r["has_bot"] else petercord
...
async def get_rawaudio_thumb(data: Dict) -> Optional[tuple]:
    key = data["id"]
    msg = data["msg"]
    client = petercord.bot if data["has_bot"] else userge
...
if petercord.has_bot:
    ...
    @petercord.bot.on_callback_query(filters.regex(pattern=r"^vcbtn_([a-z]+)$"))
    @check_owner
    ...
    @petercord.bot.on_callback_query(filters.regex(pattern=r"^vc_([a-z]+)_([-a-z+]+)$"))
    @check_owner
