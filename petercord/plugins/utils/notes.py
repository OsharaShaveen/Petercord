...
from petercord.utils import SafeDict, get_file_id, parse_buttons
...
async def get_note(message: Message) -> None:
    ...
    if can_access or is_global:
        ...
        if not message.client.is_bot and userge.has_bot and Config.INLINE_NOTES:
            ...
            try:
                x = await petercord.get_inline_bot_results(
...
async def get_inote(note_id: int, chat_id: int, user_id: int):
    message = await petercord.bot.get_messages(Config.LOG_CHANNEL_ID, note_id)
    caption = ""
    if message.caption:
    ...
    if caption:
        ...
        u_dict = await petercord.get_user_dict(user_id)
