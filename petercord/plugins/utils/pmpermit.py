...
async def allow(message: Message):
    ...
    if userid:
        ...
        if a.matched_count:
            ...
            await (await petercord.get_users(userid)).unblock()
            await message.edit("`Approved to direct message`", del_in=3)

        if userid in PMPERMIT_MSG:
            await petercord.delete_messages(userid, message_ids=PMPERMIT_MSG[userid])
...
async def get_id(message: Message):
    ...
    if message.input_str:
        ...
        try:
            userid = (await petercord.get_users(user)).id
...
async def uninvitedPmHandler(message: Message):
    """pm message handler"""
    user_dict = await petercord.get_user_dict(message.from_user.id)
...
async def outgoing_auto_approve(message: Message):
    ...
    user_dict = await petercord.get_user_dict(userID)
