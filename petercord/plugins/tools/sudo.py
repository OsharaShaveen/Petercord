...
async def add_sudo(message: Message):
    ...
    if user["id"] in Config.SUDO_USERS:
        ...
        if user["id"] in Config.OWNER_ID or user["id"] == (await petercord.get_me()).id:
            await message.edit(
...
@petercord.on_cmd("vsudo", about={"header": "view sudo users"}, allow_channels=False)
...
async def add_sudo_cmd(message: Message):
    ...
    if cmd in Config.ALLOWED_COMMANDS:
        ...
        c_d.lstrip(Config.CMD_TRIGGER) for c_d in list(petercord.manager.enabled_commands)
    ):
...
@petercord.on_cmd("vscmd", about={"header": "view sudo cmds"}, allow_channels=False)
