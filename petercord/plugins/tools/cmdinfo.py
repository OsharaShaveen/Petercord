...
async def see_info(message: Message):
    ...
    cmd_list = list(petercord.manager.commands)
    found = True
    ...
    if branch == "master":
        branch = "petercord"
    plugin_name = petercord.manager.commands[cmd_str].plugin_name
    plugin_loc = ("/" + petercord.manager.plugins[plugin_name].parent).replace(
        "/plugins", ""
    )
    if plugin_loc == "/xtra":
        extra_plugins = (
            "https://github.com/IlhamMansiez/PetercordPlugins/blob/master/plugins/"
        )
        ...
        plugin_link = "{}/blob/{}/petercord/plugins{}/{}.py".format(
            Config.UPSTREAM_REPO, branch, plugin_loc, plugin_name
        )
    local_path = f"petercord/plugins{plugin_loc}/{plugin_name}.py"
...
if petercord.has_bot:
    ...
    @petercord.bot.on_callback_query(filters.regex(pattern=r"^plugin_upload$"))
    @check_owner
    async def plugin_upload_(c_q: CallbackQuery):
        if match := plugin_regex.search(c_q.message.text):
            if os.path.exists(plugin_loc := match.group(1)):
                ...
                await petercord.bot.send_chat_action(
                    c_q.message.chat.id, "upload_document"
                )
                await petercord.bot.send_document(
