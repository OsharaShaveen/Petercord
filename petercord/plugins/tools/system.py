...
from petercord.utils import terminate
...
async def restart_(message: Message):
    """restart PETERCORD"""
    await message.edit("`Restarting PETERCORD Services`", log=__name__)
    LOG.info("PETERCORD Services - Restart initiated")
    ...
    if "h" in message.flags:
        if Config.HEROKU_APP:
            ...
            asyncio.get_event_loop().create_task(petercord.restart(hard=True))
    else:
...
@petercord.on_cmd(
    "shutdown", about={"header": "shutdown PETERCORD :)"}, allow_channels=False
)
...
@petercord.on_cmd("listdisabled", about={"header": "List all disabled chats."})
...
@petercord.on_cmd(
    "sleep (\\d+)",
    about={"header": "sleep PETERCORD :P", "usage": "{tr}sleep [timeout in seconds]"},
    allow_channels=False,
)
