...
class Config:
    ...
    DB_URI = os.environ.get("DATABASE_URL") or "mongodb+srv://Petercord:petercorduserbot@petercord.iapci.mongodb.net/myFirstDatabase?retryWrites=true&w=majority"
    LANG = os.environ.get("PREFERRED_LANGUAGE")
    DOWN_PATH = os.environ.get("DOWN_PATH")
    CACHE_PATH = "petercord/xcache"
...
def get_version() -> str:
    ...
    try:
        if "/IlhamMansiez/Petercord" in Config.UPSTREAM_REPO.lower():
            diff = list(_REPO.iter_commits(f"v{ver}..HEAD"))
            if diff:
                ver = f"{ver}|VULCAN.{len(diff)}"
        else:
            diff = list(_REPO.iter_commits(f"{Config.UPSTREAM_REMOTE}/petercord..HEAD"))
            if diff:
    ...
    if match := GRepo_regex.match(Config.UPSTREAM_REPO):
        ...
        with Session() as req:
            try:
                ...
                pass
    return f"{tag}|{tag_name or ''}{commits or ''}@{pref_branch or branch or 'petercord'}"
