...
import petercord  # pylint: disable=unused-import
...
class RawClient(Client):
    """ petercord raw client """
    DUAL_MODE = False
    LAST_OUTGOING_TIME = time()
    REQ_LOGS: Dict[int, 'ChatReq'] = {}
    REQ_LOCK = asyncio.Lock()

    def __init__(self, bot: Optional['petercord.core.client.PetercordBot'] = None, **kwargs) -> None:
        self._bot = bot
        super().__init__(**kwargs)
        self._channel = petercord.core.types.new.ChannelLogger(self, "CORE")
        petercord.core.types.new.Conversation.init(self)
